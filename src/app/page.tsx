    ...
    <>
      <div className="font-sans">
        ...
        <div className="relative flex flex-col md:flex-row gap-6 p-4 md:p-6 h-screen w-full bg-gray-100 overflow-hidden">
          ...
          <div className="w-full md:w-[70%] h-full overflow-hidden relative rounded-3xl shadow-2xl bg-black flex justify-center items-center ring-1 ring-black/5">
            ...
            {capturedImage ? (
              <img
                src={capturedImage}
                alt="Captured"
                className={`absolute inset-0 h-full w-full object-cover z-0 ${isAnalyzing ? 'blur-2xl' : 'blur-none'}`}
              />
            ) : (
              <video
                ref={videoRef}
            ...
            <canvas
              ref={canvasRef}
              className={`absolute inset-0 h-full w-full object-cover z-0 ${isAnalyzing ? 'hidden' : 'block'} ${analysis ? 'block' : 'hidden'}`}
            />
              ...
              <div className="pointer-events-none absolute inset-0 z-20">
                <div
                  ...
                  style={{
                    ...
                    maskImage: "radial-gradient(ellipse 32% 42% at center, transparent 98%, black 100%)",
                    WebkitMaskImage: "radial-gradient(ellipse 32% 42% at center, transparent 98%, black 100%)"
                  }}
                />
                <div
                  className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 border-4 transition-colors duration-300`}
                  style={{
                    width: "65%",
                    height: "85%",
                    borderRadius: "50%",
                    borderColor: isFaceInside ? "#22c55e" : "#ef4444"
                  }}
                />
              </div>
            )}
            ...
            {capturedImage ? (
              ...
              <button
                ...
                className={`absolute bottom-6 left-1/2 -translate-x-1/2 z-30 bg-white p-4 rounded-2xl shadow-lg hover:scale-105 transition flex items-center gap-2 text-gray-800 font-semibold ${isAnalyzing ? 'hidden' : 'block'}`}
              >
                {loading ? "Capturing..." : <Camera className="w-6 h-6" />}
              </button>
            )}
          </div>
                ...
                <>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {[
                      ...
                      const progressBg = rating > 80 ? 'bg-green-600' : rating > 50 ? 'bg-yellow-500' : 'bg-red-500';

                      return (
                        <div
                          key={item.key}
                          className={`bg-white rounded-xl p-3 pt-10 pb-10 shadow-sm border border-gray-100 hover:shadow-md transition-shadow ${
                            item.key === "chin" ? "md:col-span-2 md:mx-auto md:w-1/2" : ""
                          }`}
                        >
